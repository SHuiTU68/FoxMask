...
/// 移植自 WeaveMask (github.com/Seyud/WeaveMask) 的 EffectiveSystemMonetColors.kt。
///
/// 在 Monet 模式且无 keyColor（用系统壁纸色）时，直接读取 Android 12+ 框架的
/// system_* 颜色资源（system_primary_light/dark、system_surface_container_high_* 等），
/// 映射到 miuix Colors。这样能反映 root 模式下被伪造/覆盖的 overlay。
///
/// miuix 库自带的 platformDynamicColors 读 Settings.Secure overlay JSON + system_accent
/// tone 表，与本文件直读 system_*_role 资源是两条不同路径。WeaveMask 选择本文件方案
/// 以保证 root 伪造 overlay 也能生效。
@Composable
...
private fun mapFrameworkRolesToMiuixColors(roles: FrameworkMonetRoles, dark: Boolean): Colors {
    ...
    return Colors(
        ...
        tertiaryContainerVariant = roles.tertiaryContainer,
