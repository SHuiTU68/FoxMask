...
// 命令行用法: kpcall <command> [args...]
//   kpcall hello                          — 检测 kpatch 是否已安装
//   kpcall kp-version                     — 获取 kpatch 版本
//   kpcall k-version                      — 获取内核版本
//   kpcall kpm-nums                       — 获取已加载 KPM 数量
//   kpcall kpm-list                       — 列出已加载 KPM 名称
//   kpcall kpm-info <name>                — 获取 KPM 详细信息
//   kpcall kpm-load <path> [args]         — 加载 KPM
//   kpcall kpm-unload <name>              — 卸载 KPM
//   kpcall kpm-control <name> <ctl_args>  — 控制 KPM
//
// superkey 已剥离：上游 KernelPatch 不再做强 superkey 校验，root 调用者
// 固定使用 "su" 作为 superkey（root-skey 模式），无需用户配置。
...
// KernelPatch 版本码（0.13.2 = 0x0D02），用于 ver_and_cmd 的高 32 位
const KP_MAJOR: u32 = 0;
const KP_MINOR: u32 = 13;
const KP_PATCH: u32 = 2;

// 固定的 superkey：root 调用者使用 "su"（root-skey 模式）
const SUPERKEY: &str = "su";

/// 构造 supercall 的 cmd 参数: (version_code << 32) | (0x1158 << 16) | (cmd & 0xFFFF)
fn ver_and_cmd(cmd: i64) -> libc::c_long {
...
unsafe fn sc_call(cmd: i64) -> libc::c_long {
    let key_c = CString::new(SUPERKEY).unwrap_or_default();
    let combined = ver_and_cmd(cmd);
    libc::syscall(NR_SUPERCALL, key_c.as_ptr(), combined)
}

unsafe fn sc_call1(cmd: i64, arg1: *const libc::c_void) -> libc::c_long {
    let key_c = CString::new(SUPERKEY).unwrap_or_default();
    let combined = ver_and_cmd(cmd);
    libc::syscall(NR_SUPERCALL, key_c.as_ptr(), combined, arg1)
}

unsafe fn sc_call2(cmd: i64, arg1: *const libc::c_void, arg2: usize) -> libc::c_long {
    let key_c = CString::new(SUPERKEY).unwrap_or_default();
    let combined = ver_and_cmd(cmd);
...
fn print_usage() {
    eprintln!("Usage: kpcall <command> [args...]");
    eprintln!("Commands:");
    eprintln!("  hello                          Check if KernelPatch is installed");
    eprintln!("  kp-version                     Get KernelPatch version");
    eprintln!("  k-version                      Get kernel version");
    eprintln!("  kpm-nums                       Get number of loaded KPMs");
    eprintln!("  kpm-list                       List loaded KPM names");
    eprintln!("  kpm-info <name>                Get KPM information");
    eprintln!("  kpm-load <path> [args]         Load a KPM");
    eprintln!("  kpm-unload <name>              Unload a KPM");
    eprintln!("  kpm-control <name> <ctl_args>  Control a KPM");
}
...
unsafe fn kpcall_main_impl(argc: i32, argv: *mut *mut c_char) -> i32 {
    ...
    // superkey 已剥离：固定使用 "su"（root-skey 模式），argv[2] 起直接作为业务参数。
    let command = unsafe {
        match get_arg(argv, 1) {
            Some(c) => c,
            None => {
    ...
    match command.as_str() {
        "hello" => {
            let ret = sc_call(SUPERCALL_HELLO);
        ...
        "kp-version" => {
            let ret = sc_call(SUPERCALL_KERNELPATCH_VER);
            if ret >= 0 {
                let ver = ret as u32;
        ...
        "k-version" => {
            let ret = sc_call(SUPERCALL_KERNEL_VER);
            if ret >= 0 {
                println!("{}", ret as u32);
                0
            } else {
        ...
        "kpm-nums" => {
            let ret = sc_call(SUPERCALL_KPM_NUMS);
            if ret >= 0 {
                println!("{ret}");
                0
            } else {
